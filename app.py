...
def verileri_yukle():
    try:
        # worksheet isimleri Google Sheet'teki sayfa isimleriyle aynı olmalı
...
with st.sidebar:
    ...
    # Personel Listesini Google Sheets'ten Çek
    personel_listesi = []
    if not df_personel.empty and "Ad Soyad" in df_personel.columns:
        personel_listesi = df_personel["Ad Soyad"].dropna().tolist()
    ...
    if st.button("Buluta Kaydet"):
        if is_no and secilen_personel != "Seçiniz":
            ...
            st.warning("Lütfen İş No ve Personel seçiniz.")

# --- ANA EKRAN ---
tab1, tab2 = st.tabs(["📊 Üretim Takvimi", "👤 Personel Yönetimi"])
...
with tab1:
    if not df_isler.empty:
        ...
        fig = px.timeline(df_isler, x_start="Başlangıç", x_end="Bitiş", y="Personel", 
                          color="Başlık", title="Gantt Şeması (Canlı Veri)")
        fig.update_yaxes(autorange="reversed")
        st.plotly_chart(fig, use_container_width=True)
        
        st.subheader("📋 Tüm İş Listesi")
        st.dataframe(df_isler, use_container_width=True)
    else:
        st.info("Henüz kayıtlı iş yok. Yan taraftan yeni bir iş ekleyebilirsiniz.")
...
with tab2:
    ...
    if st.button("Personeli Kaydet"):
        if p_ad:
            ...
            st.success(f"{p_ad} başarıyla kaydedildi!")
            st.rerun()
        else:
            st.warning("Lütfen personel adı giriniz.")
    
    st.divider()
    st.subheader("Mevcut Personeller")
    if not df_personel.empty:
        st.table(df_personel)
